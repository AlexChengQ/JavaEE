					...
					<c:forEach items="${page.list }" var="admin">
						<tr>
							<td height="20" bgcolor="#FFFFFF">
								<div align="center">
									<input type="checkbox" name="aid" id="aid" value="${admin.aid }" />
								</div>
							</td>
						<td height="20" bgcolor="#FFFFFF" class="STYLE6">
							<div align="center">
								<span class="STYLE19">${admin.aid}</span>
							</div>
						</td>
						<td height="20" bgcolor="#FFFFFF" class="STYLE19">
							<div align="center">${admin.username}</div>
						</td>
						<td height="20" bgcolor="#FFFFFF" class="STYLE19">
...
<c:if test="${page.currentPageNum != 1}">
											<a href="${pageContext.request.contextPath }/AdminServlet?op=findAllAdmin&num=${page.previousPageNum}">上一页</a>
</c:if></span>
										</div>
									</td>
...
<c:if test="${page.currentPageNum < page.totalPageNum}">
	...
	<script type="text/javascript">
		...
		function jump() {
			...
			if (!/^[1-9][0-9]*$/.test(num)) {
				alert("请输入正确的页码");
				window.location.href = "${pageContext.request.contextPath }/CategoryServlet?op=findAllCategory&num=1";
				return;
			}
