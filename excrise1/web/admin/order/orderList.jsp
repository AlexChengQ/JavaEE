...
<c:if test="${page.currentPageNum != 1}">
											<a href="${pageContext.request.contextPath }/OrderServlet?op=findAllOrder&num=${page.previousPageNum}">上一页</a>
</c:if></span>
										</div>
									</td>
...
<c:if test="${page.currentPageNum < page.totalPageNum}">
	...
	<script type="text/javascript">
		...
		function jump() {
			...
			if (!/^[1-9][0-9]*$/.test(num)) {
				alert("请输入正确的页码");
				window.location.href = "${pageContext.request.contextPath }/OrderServlet?op=findAllOrder&num=1";
				return;
			}
